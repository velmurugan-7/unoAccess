...
// import { generateAccessToken, generateRefreshToken, verifyRefreshToken, generateRandomToken, getRefreshTokenExpiry } from '../utils/jwt';
// import { sendVerificationEmail, sendPasswordResetEmail, sendSuspiciousLoginEmail, sendAccountLockedEmail } from '../utils/email';
...
//   res.status(201).json({ success: true, message: 'Registration successful. Please check your email to verify your account.', userId: user._id });
// };

// // GET /api/auth/verify-email/:token
// export const verifyEmail = async (req: Request, res: Response): Promise<void> => {
//   const { token } = req.params;
//   const user = await User.findOne({ verificationToken: token, verificationTokenExpiry: { $gt: new Date() } }).select('+verificationToken +verificationTokenExpiry');
//   if (!user) throw new AppError('Invalid or expired verification token', 400);
//   user.isVerified = true;
//   user.verificationToken = undefined;
//   user.verificationTokenExpiry = undefined;
//   await user.save();
//   await logAudit({ userId: user._id.toString(), action: 'user.verify_email', req });
//   try {
//     const sessionMeta = await buildSessionMeta(req);
//     const refreshToken = generateRefreshToken(user._id.toString());
//     await RefreshToken.create({ token: refreshToken, userId: user._id, expiresAt: getRefreshTokenExpiry(), ...sessionMeta });
...
//   } catch { res.json({ success: true, message: 'Email verified. Please log in.' }); }
// };
...
//         await logAudit({ userId: user._id.toString(), action: 'user.account_locked', req });
//       }
//     }
//     throw new AppError('Invalid email or password', 401);
//   }

//   if (user.isLocked()) throw new AppError('Account temporarily locked. Try again in 30 minutes or reset your password.', 403);
...
//     const verified = speakeasy.totp.verify({ secret, encoding: 'base32', token: totpCode, window: 1 });
//     if (!verified) {
//       // Try backup codes
...
//     await user.save();
//     await logAudit({ userId: user._id.toString(), action: 'user.suspicious_login', req, metadata: { ip: ipHash } });
//   }
...
//   dispatchWebhook(undefined, 'user.login', { userId: user._id.toString(), email: user.email }).catch(() => {});

//   res.json({ success: true, message: 'Login successful', user: { id: user._id, name: user.name, email: user.email, role: user.role } });
// };
...
//   if (!storedToken || storedToken.expiresAt < new Date()) throw new AppError('Invalid or expired refresh token', 401);
...
//   await RefreshToken.create({ token: newRefreshToken, userId: user._id, expiresAt: getRefreshTokenExpiry(), ...sessionMeta });
...
//     dispatchWebhook(undefined, 'user.logout', { userId: req.user.userId }).catch(() => {});
//   }
...
//   await logAudit({ userId: user._id.toString(), action: 'user.password_reset', req });
...
//   const { email } = req.body;
//   const user = await User.findOne({ email });
//   if (!user || user.isVerified) { res.json({ success: true, message: 'If that email exists and is unverified, a link has been sent.' }); return; }
...
//   const secret = speakeasy.generateSecret({ name: `UnoAccess (${user.email})`, issuer: 'UnoAccess' });
...
//   if (!pendingSecret) throw new AppError('No pending 2FA setup. Start setup again.', 400);
//   const verified = speakeasy.totp.verify({ secret: pendingSecret, encoding: 'base32', token: totpToken, window: 1 });
...
//   await logAudit({ userId: user._id.toString(), action: 'user.2fa_enable', req });
//   const { sendTwoFactorBackupEmail } = await import('../utils/email');
//   await sendTwoFactorBackupEmail(user.email, user.name, backupCodes);
//   res.json({ success: true, message: '2FA enabled. Backup codes sent to your email.', backupCodes });
// };
...
//   await user.save();
//   await logAudit({ userId: user._id.toString(), action: 'user.2fa_disable', req });
